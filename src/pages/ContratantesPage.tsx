...
import { Users, Plus, Search, Edit2, Trash2, Building, User, MapPin, Phone, Mail, Check, AlertCircle, Loader2 } from 'lucide-react';
import { getContratantes, createContratante, updateContratante, deleteContratante } from '../services/contratantesService';
import { buscarCep, buscarCnpj } from '../services/viaCepService';
import { PartesDados, TipoPessoa } from '../types';
import { useAuth } from '../context/AuthContext';
import { formatarCPF, formatarCNPJ, formatarCEP } from '../utils/formatters';

interface ContratantesPageProps {
  onNavigate?: (page: string, id?: string) => void;
}

export const ContratantesPage: React.FC<ContratantesPageProps> = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateContratante(editingId, payload);
      } else {
        await createContratante(payload, currentUser?.uid || 'user');
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900/80 p-5 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm backdrop-blur-xs transition-colors duration-200">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Gerencie contratantes Pessoa Física e Jurídica com busca e autopreenchimento por CEP e CNPJ
          </p>
        </div>
        <button
      ...
      {/* Contratantes Table / Cards */}
      {loading ? (
        <p className="text-xs text-zinc-400 text-center py-8">Carregando contratantes...</p>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl p-8 text-center text-xs text-zinc-400">
          Nenhum contratante encontrado. Clique em &quot;Novo Contratante&quot; para cadastrar.
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl max-w-2xl w-full p-6 my-8 max-h-[90vh] overflow-y-auto">
            <h3 className="text-base font-bold text-zinc-900 dark:text-zinc-100 mb-4 pb-2 border-b border-zinc-100 dark:border-zinc-800">
              {editingId ? 'Editar Contratante' : 'Novo Contratante'}
            </h3>
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
              ...
              {formData.tipoPessoa === 'PF' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-2/3 border border-zinc-300 dark:border-zinc-700 rounded-lg p-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100"
                      />
                      <input
                        type="text"
                        placeholder="SSP/SP"
                        ...
                        className="w-1/3 border border-zinc-300 dark:border-zinc-700 rounded-lg p-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100"
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-1/2 border border-zinc-300 dark:border-zinc-700 rounded-lg p-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100"
                      />
                      <input
                        type="text"
                  ...
                  <div>
                    <label className="block font-bold text-zinc-700 dark:text-zinc-300 mb-1">Profissão</label>
                    <input
                      type="text"
                      placeholder="Ex: Administrador(a)"
              ...
              {formData.tipoPessoa === 'PJ' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-3 bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-100 shrink-0 font-semibold"
                      >
                        {lookupLoading ? '...' : 'Buscar CNPJ'}
                      </button>
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block font-bold text-zinc-700 dark:text-zinc-300 mb-1">Inscrição Estadual/Municipal</label>
                    <input
                      type="text"
                      placeholder="IE / IM"
                  ...
                  {/* Representante Legal Sub-Section */}
                  <div className="sm:col-span-2 p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-lg border border-zinc-200 dark:border-zinc-700 space-y-2">
                    <p className="font-bold text-zinc-800 dark:text-zinc-200">Representante Legal da Empresa</p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      <input
                        ...
                        onChange={e => setFormData({
                          ...formData,
                          representanteLegal: { ...formData.representanteLegal!, nome: e.target.value }
                        })}
                        className="border border-zinc-300 dark:border-zinc-700 rounded-md p-1.5 bg-white dark:bg-zinc-800"
                      />
                      <input
                        type="text"
                        ...
                        onChange={e => setFormData({
                          ...formData,
                          representanteLegal: { ...formData.representanteLegal!, cpf: formatarCPF(e.target.value) }
                        })}
                        className="border border-zinc-300 dark:border-zinc-700 rounded-md p-1.5 bg-white dark:bg-zinc-800"
                      />
                      <input
                        ...
                        onChange={e => setFormData({
                          ...formData,
                          representanteLegal: { ...formData.representanteLegal!, cargo: e.target.value }
                        })}
                        className="border border-zinc-300 dark:border-zinc-700 rounded-md p-1.5 bg-white dark:bg-zinc-800"
                      />
                    </div>
              ...
              <div className="pt-2 border-t border-zinc-100 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={lookupLoading}
                      className="px-2.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 border border-zinc-300 dark:border-zinc-700 rounded-lg font-semibold shrink-0"
                    >
                      {lookupLoading ? '...' : 'CEP'}
                    </button>
                  </div>
                ...
                <div className="sm:col-span-2">
                  <label className="block font-bold text-zinc-700 dark:text-zinc-300 mb-1">Endereço (Rua/Avenida)</label>
                  <input
                    type="text"
